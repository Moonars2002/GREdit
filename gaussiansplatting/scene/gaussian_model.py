...
class GaussianModel:
    ...
    def __init__(
        ...
        # EMA residual score for edit-aware densification gate
        self.edit_score = torch.empty(0)
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        # Initialize edit_score
        self.edit_score = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
    ...
    def prune_points(self, mask):
        ...
        # Synchronize pruning for edit_score
        if self.edit_score.numel() > 0:
            self.edit_score = self.edit_score[valid_points_mask]
    ...
    def densification_postfix(
        ...
        # Append zeros to edit_score for newly added points
        n_new = new_xyz.shape[0]
        if self.edit_score.numel() == 0:
            n_old = self.get_xyz.shape[0] - n_new
            self.edit_score = torch.zeros((n_old, 1), device="cuda")

        self.edit_score = torch.cat(
            [self.edit_score, torch.zeros((n_new, 1), device="cuda")], dim=0
        )

    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, edit_gate=None):
        ...
        # Filter points using edit_gate.
        if edit_gate is not None:
            assert edit_gate.shape[0] == selected_pts_mask.shape[0]
            selected_pts_mask = torch.logical_and(selected_pts_mask, edit_gate)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, edit_gate=None):
        # Extract points that satisfy the gradient condition
    ...
    def densify_and_prune(
            self, max_grad, max_densify_percent, min_opacity, extent, max_screen_size, edit_densify_enabled: bool = False,edit_densify_thres: float = 0.0,
    ):
        ...
        # Densify only points with high edit_score within the optimizable mask
        edit_gate = None
        if edit_densify_enabled and self.edit_score.numel() > 0:
            edit_gate = (self.edit_score.squeeze(-1) >= edit_densify_thres)
            edit_gate = torch.logical_and(edit_gate, self.mask)

        before = self.get_xyz.shape[0]

        if edit_gate is not None:
            edit_gate = edit_gate[:before]

        self.densify_and_clone(grads, max_grad, extent, edit_gate=edit_gate)
        clone = self.get_xyz.shape[0]

        # Pad edit_gate with False if cloning generated new points
        if edit_gate is not None:
            current_points_num = self.get_xyz.shape[0]
            gate_points_num = edit_gate.shape[0]
            
            if current_points_num > gate_points_num:
                num_new_points = current_points_num - gate_points_num
                padding = torch.zeros(num_new_points, device="cuda", dtype=torch.bool)
                edit_gate = torch.cat([edit_gate, padding], dim=0)

        self.densify_and_split(grads, max_grad, extent, edit_gate=edit_gate)
    ...
    @torch.no_grad()
    def update_edit_score(self, per_gaussian_score: torch.Tensor, ema: float = 0.9):
        """
        per_gaussian_score: [N,1] or [N], float, Higher value means higher editing residual
        """
        if per_gaussian_score.ndim == 1:
            per_gaussian_score = per_gaussian_score[:, None]
        if self.edit_score.numel() == 0 or self.edit_score.shape[0] != per_gaussian_score.shape[0]:
            self.edit_score = torch.zeros_like(per_gaussian_score)
        self.edit_score = ema * self.edit_score + (1.0 - ema) * per_gaussian_score
