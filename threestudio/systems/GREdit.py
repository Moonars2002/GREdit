...
@threestudio.register("gredit-system")
class GREdit(BaseLift3DSystem):
    @dataclass
    class Config(BaseLift3DSystem.Config):
        ...
        # RGD related configurations
        edit_densify: bool = True
        edit_residual_quantile: float = 0.9
        edit_score_ema: float = 0.9
        edit_densify_thres: float = 0.3
        edit_score_update_interval: int = 50
        edit_score_use_local_mask: bool = True 
    ...
    def configure(self) -> None:
        ...
        # 在 configure() 末尾加
        self._last_residual_mask = None
        self._last_batch_cam_indices = None
        ...
        with torch.no_grad():
            if self.true_global_step < self.cfg.densify_until_iter:
                ...
                if (
                    ...
                    # edit_score
                    if (
                        self.cfg.edit_densify
                        and self._last_residual_mask is not None
                        and self._last_batch_cam_indices is not None
                        and (self.true_global_step % self.cfg.edit_score_update_interval == 0)
                    ):
                        weights = torch.zeros_like(self.gaussian._opacity)
                        weights_cnt = torch.zeros_like(self.gaussian._opacity, dtype=torch.int32)

                        for b, cam_id in enumerate(self._last_batch_cam_indices):
                            cam = self.trainer.datamodule.train_dataset.scene.cameras[cam_id]
                            img_w = self._last_residual_mask[b].permute(2, 0, 1)  # [1,H,W] 适配 apply_weights
                            self.gaussian.apply_weights(cam, weights, weights_cnt, img_w)

                        score = weights / (weights_cnt + 1e-7)  # [N,1] 0~1
                        self.gaussian.update_edit_score(score, ema=self.cfg.edit_score_ema)
                    ...
                    self.gaussian.densify_and_prune(
                        ...
                        self.cameras_extent,
                        5,
                        edit_densify_enabled=self.cfg.edit_densify,
                        edit_densify_thres=self.cfg.edit_densify_thres,
                    )
    ...
    def edit_all_view(self, original_render_name, cache_name, update_camera=False, global_step=0):
        ...
        with torch.no_grad():
            ...
            for id in view_sorted:
                ...
                # Collect depth map
                all_depths.append(out_pkg["depth"])
            ...
            # Concatenate depth maps for structural guidance
            all_depths = torch.cat(all_depths, dim=0) # [Total_Views, H, W, 1]
    ...
    def training_step(self, batch, batch_idx):
        if self.true_global_step % self.cfg.camera_update_per_step == 0 and self.cfg.guidance_type == 'gredit-guidance' and not self.cfg.loss.use_sds:
        ...
        if self.cfg.guidance_type == 'gredit-guidance': 
        ...
        # Train Adapter at early stages
        if self.cfg.guidance_type == 'gredit-guidance' and not self.cfg.loss.use_sds and self.true_global_step < self.cfg.camera_update_per_step:
            
            current_depth = out["depth"].detach()
            
            current_gt_rgb = []
            for idx in batch["index"]:
                idx_val = idx.item() if isinstance(idx, torch.Tensor) else idx
                current_gt_rgb.append(self.origin_frames[idx_val])
            current_gt_rgb = torch.cat(current_gt_rgb, dim=0) # [B, H, W, 3]

            loss_adapter = self.guidance.train_adapter_step(
                current_gt_rgb, 
                current_depth, 
                self.prompt_processor()
            )
            
            self.log("train/loss_adapter", loss_adapter)
        ...
        if self.cfg.loss.lambda_l1 > 0 or self.cfg.loss.lambda_p > 0:
            ...
            with torch.no_grad():
                # images: [B,H,W,3], gt_images: [B,H,W,3]
                res_map = (images - gt_images).abs().mean(dim=-1, keepdim=True)  # [B,H,W,1]

                if self.cfg.edit_score_use_local_mask:
                    res_map = res_map * out["masks"].unsqueeze(-1).float()

                # Adaptive quantile thresholding for residuals
                thr = torch.quantile(res_map.reshape(-1), self.cfg.edit_residual_quantile)
                self._last_residual_mask = (res_map > thr).float()
                self._last_batch_cam_indices = [
                    (i.item() if isinstance(i, torch.Tensor) else int(i)) for i in batch_index
                ]
