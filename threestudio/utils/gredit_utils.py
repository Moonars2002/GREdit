...
def register_extended_attention(model):
    def sa_forward(self):
        ...
        def forward(x, encoder_hidden_states=None, attention_mask=None):
            ...
            # Fallback: Use standard Self-Attention for non-multi-view batches
            if batch_size % 3 != 0:
                q = self.to_q(x)
                k = self.to_k(x)
                v = self.to_v(x)
            ...
            if hasattr(self, "adapter_features") and self.adapter_features is not None:
                feat = self.adapter_features # [3*N, C, H, W]
                
                if feat.shape[1] == dim:
                    d_feat = feat[n_frames : 2*n_frames] # [N, C, H, W]
                    B_d, C_d, H_d, W_d = d_feat.shape
                    
                    d_flat = d_feat.view(B_d, C_d, -1).permute(0, 2, 1)
                    
                    d_flat = torch.nn.functional.normalize(d_flat, p=2, dim=-1)

                    d_key = d_flat.view(1, -1, C_d).repeat(B_d, 1, 1)
                    
                    bias_val = torch.bmm(d_flat, d_key.transpose(1, 2))
                    
                    depth_bias = bias_val.repeat_interleave(h, dim=0)
            ...
            for j in range(h):
                ...
                # Apply Time-Aware Depth Bias (Square decay schedule, truncating at last 25%)
                if depth_bias is not None:
                    base_scale = getattr(self, "depth_bias_scale", 0.2)
                    
                    t_cur = getattr(self, "diffusion_t", 0)       
                    t_total = getattr(self, "diffusion_t_total", 1) 
                    
                    ratio = float(t_cur) / float(max(t_total, 1)) 
                    
                    dynamic_weight = ratio ** 2
                    
                    if ratio < 0.25:
                        dynamic_weight = 0.0
                    
                    final_scale = base_scale * dynamic_weight

                    bias_slice = depth_bias[j::h] 

                    if final_scale > 1e-4:
                        sim_image = sim_image + bias_slice * final_scale
...
def make_gredit_block(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:

    class GREditBlock(block_class):
        def forward(
            self,
            hidden_states,
            attention_mask=None,
            ...
            # Fallback to original forward pass for non-multi-view batches
            if hasattr(self, "adapter_features") and self.adapter_features is not None:
                if hidden_states.shape[-1] == self.adapter_features.shape[1]: 
                    B, C, H, W = self.adapter_features.shape
                    feat = self.adapter_features.permute(0, 2, 3, 1).reshape(B, -1, C)
                    
                    hidden_states = hidden_states + feat

            if hidden_states.shape[0] % 3 != 0:
...
# Depth Adapter and Auxiliary Functions
class DepthAdapter(torch.nn.Module):
    def __init__(self, channels=[32, 64, 128, 320]):
        super().__init__()
        self.conv_in = torch.nn.Conv2d(1, channels[0], 3, 1, 1)
        self.blocks = torch.nn.ModuleList()
        for i in range(len(channels)-1):
            self.blocks.append(torch.nn.Sequential(
                torch.nn.Conv2d(channels[i], channels[i+1], 3, 2, 1),
                torch.nn.SiLU(),
                torch.nn.Conv2d(channels[i+1], channels[i+1], 3, 1, 1),
                torch.nn.SiLU()
            ))
        
        # Zero-convolution to ensure identity mapping at initialization
        self.zero_conv = torch.nn.Conv2d(channels[-1], 320, 1)
        self.zero_conv.weight.data.zero_()
        self.zero_conv.bias.data.zero_()

    def forward(self, depth_map):
        # depth_map: [B, 1, H, W]
        x = self.conv_in(depth_map)
        for block in self.blocks:
            x = block(x)
        return self.zero_conv(x)

def register_adapter_features(diffusion_model, adapter_features, use_bias=True):
    for _, module in diffusion_model.named_modules():
        if isinstance_str(module, "BasicTransformerBlock"):
            setattr(module, "adapter_features", adapter_features)
            # Pass adapter features to Attn1 for Depth-Aware Attention Bias
            if hasattr(module, "attn1"):
                if use_bias:
                    setattr(module.attn1, "adapter_features", adapter_features)
                else:
                    setattr(module.attn1, "adapter_features", None) 

def register_time_info(diffusion_model, t: int, t_total: int):
    for _, module in diffusion_model.named_modules():
        if isinstance_str(module, "BasicTransformerBlock"):
            if hasattr(module, "attn1"):
                setattr(module.attn1, "diffusion_t", int(t))
                setattr(module.attn1, "diffusion_t_total", int(t_total))

def register_depth_bias_scale(diffusion_model, scale: float):
    for _, module in diffusion_model.named_modules():
        if isinstance_str(module, "BasicTransformerBlock"):
            if hasattr(module, "attn1"):
                setattr(module.attn1, "depth_bias_scale", scale)